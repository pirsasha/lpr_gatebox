...
2. **Дубли/рассинхрон UI-слоёв — закрыто (2026-02-28)**  
   Ранее в проекте параллельно лежали `ui/src/api.js` и `ui/src/api/gatebox.ts`, а также legacy-страницы камеры (`Camera.tsx`) и `QuickSetup.tsx`, не подключённые в роутинг.
   **Статус:** закрыто — удалены неиспользуемые страницы и дублирующий API-клиент; единый UI API-клиент: `ui/src/api.js`.
...
### Быстрые (1–2 часа)
- [x] Починить/удалить `fetch_settings()` в `http_client.py` (исправлено: legacy-обёртка использует `fetch_settings_json()`).
- [x] `camera/test`: реализовать реальный timeout и корректный fail-fast (реализовано через `ThreadPoolExecutor` + timeout).
- [x] В UI скрыть/пометить override-ключи, которые не hot-apply (есть классификация hot/restart/unknown и явная подсказка о рестарте `rtsp_worker`).

### Средние (1–2 дня)
- [x] Ввести explicit restart-only список ключей + UI индикацию (добавлены `rtsp_worker/capabilities` и бейджи в Settings).
...
- [x] Единый API-клиент UI (дубль `api/gatebox.ts` удалён, используется `api.js`).

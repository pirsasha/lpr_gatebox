# =========================================================
# FILE: updater/server.py
# PROJECT: LPR GateBox
# VERSION: v0.3.1
# UPDATED: 2026-02-07 (+0300)
#
# WHAT CHANGED:
# - NEW: GET /metrics -> host CPU/RAM/DISK + docker stats (gatebox/rtsp_worker/updater)
# - FIX: safer command runner + JSON responses
# - NOTE: works with docker.sock mounted into updater
# =========================================================
...
def run(cmd):
    # NOTE: keep docker-compose (legacy) as in your code
    log(f"$ {' '.join(cmd)}")
    p = subprocess.Popen(
        cmd,
        cwd=PROJECT_DIR,
...
def do_update():
    ...
    try:
        STATE["step"] = "pull"
        rc1 = run(["docker-compose", "-f", COMPOSE_FILE, "pull"])
        if rc1 != 0:
            raise RuntimeError(f"pull failed rc={rc1}")

        STATE["step"] = "restart"
        rc2 = run(["docker-compose", "-f", COMPOSE_FILE, "up", "-d"])
        if rc2 != 0:
...
def _read_meminfo_kb():
    # returns (mem_total_kb, mem_avail_kb)
    mem_total = None
    mem_avail = None
    try:
...
def _read_cpu_percent(interval_sec: float = 0.15) -> float:
    """
    Approx CPU usage percent via /proc/stat (works in container scope).
    For real host/container-specific stats we also return docker stats below.
    """
    def snap():
        try:
...
def _docker_stats():
    """
    docker stats one-shot. Requires docker.sock mounted.
    Returns per container: name, cpu_pct, mem_used, mem_limit.
    """
    try:
        cmd = [
            "docker",
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        if self.path == "/check":
            # пока "unknown", дальше подключим GitHub release/tags
...
log(f"updater starting on :{PORT} project={PROJECT_DIR} compose={COMPOSE_FILE}")
